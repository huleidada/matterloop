...
除 `OtelExporter` 自带 SDK 与 OTLP/HTTP Exporter 外，OpenTelemetry 的 Provider、Exporter、采样与
资源属性由应用配置。Team 事件会携带完整 Snapshot，写入 SIEM 或 Trace 前要评估体积、基数与敏感字段。

需要树形 Trace 时，用 `TraceBuilder` 搭配 `BatchingPipeline` 把事件流重建为跨度树并提取验证评分，
模型客户端用 `TracedModelClient` 包装后自动记录 generation 跨度；production preset 传入
`trace_exporter` 可完成同样装配，导出流水线随 runtime 关闭自动排空。装配细节见
